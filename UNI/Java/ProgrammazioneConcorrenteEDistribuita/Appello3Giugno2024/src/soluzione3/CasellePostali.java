...
public class CasellePostali extends UnicastRemoteObject implements GestoreInt {
	...
	private List<ClientInt> clientsRegistrati;
	
	public CasellePostali() throws RemoteException {
		super();
		messaggi = new Hashtable<String, Messaggio>(); // Sarebbe già thread safe
		clients = new ArrayList<>(); // Cambiato in variante thread safe
		clientsRegistrati = new CopyOnWriteArrayList<>();
	}
	...
	public synchronized void put(Messaggio m) {
		...
		if (clients.contains(dest)) {
			...
			show();
			notifyAll();
			for (ClientInt clientCoso : clientsRegistrati) {
				try {
					clientCoso.notifica(m);
	...
	@Override
	public void registra(ClientInt c) throws RemoteException {
		clientsRegistrati.add(c);
	}

	@Override
	public void deregistra(ClientInt c) throws RemoteException {
		clientsRegistrati.remove(c);
	}
}
