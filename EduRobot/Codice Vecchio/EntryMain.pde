...
[ Z ] Cucchiaio / Approcciare la palla in direzione corretta
***********************/
...
void m_centrati(), lettura_schedaPalla(), lettura_bussola(), movimento_cercaPalla(), movimento_inseguiPalla(), giraFinoAngolo(int angolo), BEEP(), movimento_emergenza();
...
void loop() {
    ...
    */
    ...
    letturaSelettore1 = analogRead(SELETTORE1) + 65;
    letturaSelettore2 = analogRead(SELETTORE2) + 65;
    //Serial.write(letturaSelettore1);
    //Serial.write('-');
    //Serial.write(letturaSelettore2);
    
    if(letturaSelettore1 == 'A' && letturaSelettore2 == 'A'){
        if(digitalRead(PUSH) == 0) {
            digitalWrite(USERLED, LOW);
            delay(50);
            digitalWrite(USERLED, HIGH);
            if(emergenza == true) emergenza = false;
            else if(emergenza == false) emergenza = true;
            delay(500);
        }
        if(emergenza == true) {
            movimento_emergenza();
        } else m_centrati();
    } else if(letturaSelettore1 == 'A' && letturaSelettore2 == '@'){
         if(lato_palla() == 1 && lato_bussola() == 0) m_avanti();
         else {
             if(DIST >= 2) {
                ...
                if(emergenza  == false) {
                    movimento_inseguiPalla();
                    emergenza = true;
                }
                else if (emergenza == true) movimento_emergenza();
            }
            //movimento_inseguiPalla();
         }
    } else if(letturaSelettore1 == '@' && letturaSelettore2 == 'A'){
        if(!fatto){
            fatto = true; //Va effettuata solo una volta
            m_avanti(); //Vado avanti per 200 millisecondi
            delay(300);
            m_fermo(); //Mi fermo
            
            Serial.write('f'); //Do il via all'altro robot
            
            delay(200);
            m_indietro(); //Torno indietro
            delay(100);
            m_fermo();
        }
    } else if(letturaSelettore1 == '@' && letturaSelettore2 == '@'){
        if(fatto) {
            //if(emergenza  == false) movimento_inseguiPalla();
            movimento_emergenza();
        }
    ...
    /*****COMUNICAZIONE BLUETOOTH DIMOSTRAZIONE****/
    //Comunicazione bluetooth
     /******RICEZIONE*****/
     /*
    if(bluetoothDimostrazione == 1){   
        if(Serial.available()){
            datoSeriale = Serial.read();
            switch(datoSeriale){
                case 42:    //Cambio ruoli
                    bluetoothDimostrazione = 2;
                    m_fermo();
                    break;
                case 65:    //A
                    m_avanti();
        ...
        /*
    } else if(bluetoothDimostrazione == 2){   
         lettura_bussola();
         if(digitalRead(PUSH) == 0){
             Serial.write('*');
             Serial.write('*');
             Serial.write('*');
             bluetoothDimostrazione = 1;
         } else {
         if(BUS <= 32 || BUS >= 241) Serial.write('A');            //Avanti
         else if (BUS >= 33 && BUS <= 100) Serial.write('D');     //Sinistra
         else if(BUS >= 101 && BUS <= 190) Serial.write('R');    //Retro
         else if(BUS >= 191 && BUS <= 240) Serial.write('S');    //Destra
         }
         delay(50);   //Delay per non intasare la seriale
    }
...
void vaiAPortaNemica(int angolo){
    angolo = calcola_angoloTiro();    //Imposto come angolo da raggiungere la porta. SE NON VA METTI calcola_angoloTiro
    rangeAngoloSX = 240;
    rangeAngoloDX = 15;
    lettura_bussola();
    while((BUS > rangeAngoloSX && BUS < rangeAngoloDX) == false){    //Finchè non sono nel range considerato angolo, mi ruoto
...
//              }
             ...
             giraFinoAngolo(64);
             ciclo = 0;
             while(lato_palla() != 3 && ciclo < tempoCiclo) {
                   ciclo++;
                   m_avanti();
                   delay(5);
                   m_fermo();
             ...
             ciclo = 0;
         } else if(lato_bussola() == 2 || lato_bussola() == 3){
             giraFinoAngolo(160);
             delay(2);
             giraFinoAngolo(160);
             delay(500);
             ciclo = 0;
             while(lato_palla() != 2 && ciclo < tempoCiclo) {
                 ciclo++;
                 m_avanti();
                 delay(5);
                 m_fermo();
...
void movimento_cercaPalla(){
    /*****FUNZIONE DI RICERCA DELLA PALLA (NON UTILIZZARE)*****/
    ...
    while(1){
        ...
        if(lato_bussola() == 0) {
            ...
            return;    //Mi sono centrato
                ...
                m_ruota_sx();       //Giro a sinistra
                delay(5);
                m_fermo();
        }
        else if(lato_bussola() == 3 || lato_bussola() == 4) {
                m_ruota_dx();       //Giro a sinistra
                delay(5);
                m_fermo();
        }
    }
