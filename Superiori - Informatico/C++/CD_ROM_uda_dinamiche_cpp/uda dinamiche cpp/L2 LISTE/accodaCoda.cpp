...
int main(){
  ...
  // creazione coda
  creaCoda (&pTesta, &pFine);
  // creaCodaRandom (&pTesta, &pFine, tanti);
  inCoda (&pTesta, &pFine, 1);
  inCoda (&pTesta, &pFine, 2);
  inCoda (&pTesta, &pFine, 3);
  inCoda (&pTesta, &pFine, 4);
...
/* Scopo: creazione manuale di una coda       */ 
/* ==> con le funzioni dilibreria             */ 
