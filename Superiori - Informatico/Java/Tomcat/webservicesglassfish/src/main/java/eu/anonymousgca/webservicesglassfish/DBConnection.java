...
public class DBConnection {
    ...
    private static final String DB_CONNECTION = "jdbc:mysql://172.22.201.51:3306/AnonymousGCAChatDB";
    ...
    public boolean autenticazione(String email, String password){
        ...
        try {
            Statement stmt = dbConnection.createStatement();
            String select = "SELECT * FROM Utente WHERE email='" + email + "' AND password='" + password + "';";
            rs = stmt.executeQuery(select);
        } catch (SQLException e) {
            System.out.println("Errore esecuzione query (dati inseriti: " + email + " e " + password);
            return false;
        }
        ...
        try {
            if (rs.next()){
                ...
                System.out.println("Utente trovato con email: " + email + " e password: " + password);

                // Modifico ultimo accesso con adesso
                String update = "UPDATE Utente SET ultimo_accesso = NOW() WHERE id_utente = '" + email + "';";
                Statement stmt = dbConnection.createStatement();
